...
Выгрузка видеоконтента | (666 000 / (24 * 3600)) / (7.4 * 10 ^ 6 / (24 * 3600)) * 16.3 = **1.3** Мбит / с 
...
Выгрузка видеоконтента | 1.3 * 60 * 45 * (7.4 * 10 ^ 6 / (24 * 3600)) = **0.29** Тбит / c
...
Тогда, учитывая, что одно ядро процессора может отдать ~50rps, нам понадобится 2 сервера с 32 ядрами каждый.
...
равномерно распределены между тремя бэкендами для обеспечения отказоустойчивости. А запросы за видео будут проксироваться на CDN-сервера. Загрузка видео на
...
запрашивать статистику о наиболее популярных видео. После того как сервис загрузки контента проанализирует статистику,
будет осуществляться размещение наиболее популярного контента в кластерах, роутинг между которыми будет реализован
с использованием [geo based dns](https://docs.microsoft.com/ru-ru/windows-server/networking/dns/deploy/primary-geo-location).
...
|---                        |---   |---    |---                    |---         |---
|Бэкенд                     |32    | 256 | 32Гб х 1SSD           | 3+6        |Москва и Санкт-Петербург     
|Фронтенд                   |16    | 256 | 32Гб х 1SSD           | 2+4        |Москва и Санкт-Петербург     
|Postgresql                 |32    | 512 | 1Тб х 1SSD            | 1+2        |Москва и Санкт-Петербург 
|Nginx                      |32    | 256 | 16Гб х 1SSD           | 14+7       |Москва и Санкт-Петербург
|Сервис рекомендаций        |8     | 256 | 32Гб х 1SSD           | 4+2        |Москва и Санкт-Петербург
|Сервис загрузки контента   |8     | 128 | 32Гб х 1SSD           | 4+2        |50 % - Москва и Санкт-Петербург 50 % - остальные крупные города России
|Сервера CDN                |8     | 64  | 64Гб х 1SSD           | 109        |50 % - Москва и Санкт-Петербург 50 % - остальные крупные города России
